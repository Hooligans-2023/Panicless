...
class ForgetPasswordCheckCodeUseCase @Inject constructor(
    private val repository: AuthRepository
) {
    private val TAG = "LoginUseCase"
    operator fun invoke(forgetPasswordCheckCodeBody: ForgetPasswordCheckCodeBody): Flow<Resource<MainResponseModel<ForgetPasswordRestResponse>>> = flow {
        try {
            emit(Resource.Loading())
            val myResponse = repository.forgetPasswordCheckCode(forgetPasswordCheckCodeBody)
            Log.d(TAG, "invoke: Login use case ${myResponse.isSuccessful}")
            if (myResponse.isSuccessful && myResponse.body() != null) {
                myResponse.body()?.let { response ->
                    emit(Resource.Success(response))
                }
            } else {
                Log.d(TAG, "invoke: Login use case errorBody ${myResponse.errorBody()}")
